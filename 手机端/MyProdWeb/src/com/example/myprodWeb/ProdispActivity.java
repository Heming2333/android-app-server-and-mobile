...
public class ProdispActivity extends Activity {
	...
	public void add(View v) {
		...
		final Account a = new Account(name, balance.equals("") ? 0: Integer.parseInt(balance),stock.equals("") ? 0: Integer.parseInt(stock));
		dao.insert(a);                      // 插入数据库
			...
			public void run() {
				...
				if ((result != null)&&result.equals("0")) {
					...
					runOnUiThread(new Runnable() {
						@Override
						public void run() {
							Toast.makeText(ProdispActivity.this, "添加失败...result="+result, 0)
									.show();
						}
					});
				}
	...
	private class MyAdapter extends BaseAdapter {
		...
		TextView stockTV = (TextView) item.findViewById(R.id.stockTV);
          // 根据当前位置获取Account对象
		 ...
		 upIV.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					new Thread() {//开启子线程访问网络
						public void run() {
			                   //调用LoginService里面的方法请求网络获取数据
							final String result = ProdDBService.updateByClientGet(a);
							...
							if ((result != null)&&result.equals("0")) {
								...
								runOnUiThread(new Runnable() {
									@Override
									public void run() {
										Toast.makeText(ProdispActivity.this, "更新失败...", 0)
												.show();
									}
								});
							}
					...
					android.content.DialogInterface.OnClickListener listener = 
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							new Thread() {//开启子线程访问网络
								public void run() {
					                   //调用LoginService里面的方法请求网络获取数据
									final String result = ProdDBService.deleteByClientGet(a.getId());
									...
									if ((result != null)&&result.equals("0")) {
										...
										runOnUiThread(new Runnable() {
											@Override
											public void run() {
												Toast.makeText(ProdispActivity.this, "注册失败...", 0)
														.show();
											}
										});
									}
